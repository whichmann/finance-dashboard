...
- `app/page.tsx` is the public landing page. `app/dashboard/layout.tsx` wraps dashboard routes with the responsive `SideNav`; dashboard pages include the overview and spending CRUD screens.
...
- Tailwind CSS is configured in `tailwind.config.ts`. Shared semantic theme tokens live in `app/ui/design-tokens.css`, while app-wide rules live in `app/ui/global.css`. Fonts are centralized in `app/ui/fonts.ts`, and the `@/*` TypeScript alias maps to the repository root.
...
- Treat `app/ui/design-tokens.css` as the public theme contract: keep token names stable, use semantic tokens in UI code, and avoid coupling microfrontends to component-specific color values.
